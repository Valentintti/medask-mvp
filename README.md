...
服务端对请求字段、长度、Origin、速率、每日预算和严格 JSON 响应做独立校验，并保留最小化审计元数据。完整说明见 [`docs/real-provider-security.md`](docs/real-provider-security.md)。
...
`pnpm eval:real` 在没有完整本机配置时安全跳过，不调用 API；配置后也只对合成文本运行三轮工程评测，不使用真实患者数据。
